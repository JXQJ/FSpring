...
class VideoListView :
	public VirtualView {
public:
	VideoListView(CWnd* wnd);
	virtual ~VideoListView();
...
protected:
	...
	void DrawProgress(CDC* pDC) {
		...
		if (g_progress != 0) {
			thumb_rect.right = static_cast<LONG>(thumb_rect.left + progress_rect.Width()*((float)g_progress / g_total_frame_count));
		} else {
		...
		CString str;
		str.Format(TEXT("extract frames ... (%lld/%lld)"), g_progress, g_total_frame_count);
